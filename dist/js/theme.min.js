...
function sliderPaddingHack() {
  ...
  offsetSliders.forEach(function (slider) {
    ...
    var sliderOffset = (sliderWidth - containerWidth) / 2;
...
;
sliderPaddingHack(); // function checkForWindowResize() {
...
function sliderArrowPosition() {
  ...
  var projectsSliderArrow = document.querySelector('.projects-slider__btn');
...
var decorRandomCircles = document.querySelectorAll('.random-circles .decor-circle');
decorRandomCircles.forEach(function (el, i, ra) {
  var to = {
...
document.addEventListener('DOMContentLoaded', function () {
  // const controls = [];
  var artistHeroVideoPlay = document.querySelector('#artistVideoPlay');
  var artistHeroVideoPause = document.querySelector('#artistVideoPause');
  var artistHeroVideoplayer = new Plyr('#artist-video', {
    // controls,
    volume: 0.5
  });

  if (artistHeroVideoPlay) {
    artistHeroVideoPlay.addEventListener('click', function () {
      artistHeroVideoplayer.fullscreen.enter();
      artistHeroVideoplayer.play(); // artistHeroVideoPlay.classList.remove('active');
  ...
  artistHeroVideoplayer.on('exitfullscreen', function () {// artistHeroVideoplayer.pause();
  });
  var videoPlayers = Plyr.setup('.video-card__video', {
    fullscreen: {
      iosNative: true
    }
  });
  ...
  var videoPlayerWrapper = document.querySelector('.video-player-big');

  if (videoPlayerWrapper) {
    var loadPlaylist = function loadPlaylist(target, allVideos, id, listclass) {
      $("li.pls-playing").removeClass("pls-playing"); // $('.video-list li:first-of-type').addClass("pls-playing");

      $(".plyr-playlist-wrapper").remove();
      limit = limit - 1;

      if (allVideos) {
        PlyrPlaylist(".player-playlist__list", allVideos, limit, id, listclass); //return
      }
      ...
      function PlyrPlaylist(target, allVideos, limit, id, listclass) {
        ...
        var videoImg = videoPlayerWrapper.querySelector('.player-list__item-img img'),
            ...
            videoBottomControlsFullscreen = videoPlayerWrapper.querySelector('.video-controls #video-full-screen'); // Timeline

        videoTimelineContainer.addEventListener("mousemove", handleTimelineUpdate);
        videoTimelineContainer.addEventListener("mousedown", toggleScrubbing); // videoTimelineContainer.addEventListener("touchmove", handleTimelineUpdate)
        ...
        function toggleScrubbing(e) {
          var rect = videoTimelineContainer.getBoundingClientRect();
        ...
        function nextVideo() {
          $(".player-playlist__list .pls-playing").next().find("a").trigger("click");
        }

        function prevVideo() {
          $(".player-playlist__list .pls-playing").prev().find("a").trigger("click");
        }
        ...
        players[0].on('volumechange', function () {
        ...
        videoMobilePlayBtn.addEventListener('click', function () {
          togglePlay(); // setTimeout(() => {
        ...
        videoVolumeSlider.addEventListener('input', function (e) {
          players[0].volume = e.target.value;
          players[0].muted = e.target === 0;
        });
        videoBottomControlsFullscreen.addEventListener('click', function () {
        ...
        players[0].on("ended", function (event) {
          $('#video-main .plyr').addClass('video-fullscreen');
          var $next = $(".player-playlist__list .pls-playing").next().find("a").trigger("click");
        });
        ...
        players[0].on('enterfullscreen', function () {
          $('#video-main .plyr').addClass('video-fullscreen'); // players[0].play();
        });
      }
      ...
      function videoControlsMediaText() {
        var playingVideoArtist = $('li.pls-playing .player-list__item-artist').text();
        var playingVideoText = $('li.pls-playing .player-list__item-media').text();
        $('.video-controls .song-name').html("<p>".concat(playingVideoText, "</p>"));
        $('.video-controls .song-media').html("<p>".concat(playingVideoArtist, "</p>"));
        $('.video-playlist-mobile .song-name').html("".concat(playingVideoText));
        $('.video-playlist-mobile .song-media').html("".concat(playingVideoArtist));
      }
      ...
      players[0].on("ready", function (event) {
        players[0].volume = .5;
        var videoDuration = players[0].duration;
        var totalVideoMin = Math.floor(videoDuration / 60);
        var totalVideoSec = Math.floor(videoDuration % 60);
        $('.video-controls .plyr__time.plyr__time--duration').html("".concat(totalVideoMin) + ':' + "".concat(totalVideoSec));
        $('.video-controls .plyr__time.plyr__time--current').text('0:00');
        $('.timeline-current-time').text('0:00');
        $('.video-controls .player-controls__bottom-thumb').html("<img src=\"".concat(players[0].poster, "\"></img>"));
        $('.video-playlist-mobile .player-controls__bottom-thumb').html("<img src=\"".concat(players[0].poster, "\"></img>"));
        videoControlsMediaText();
      });
      players[0].on("timeupdate", function (event) {
        $('.video-controls .player-controls__bottom-thumb').html("<img src=\"".concat(players[0].poster, "\"></img>"));
        var currentVideoMin = Math.floor(players[0].currentTime / 60);
        var currentVideoSec = Math.floor(players[0].currentTime % 60);

        if (currentVideoSec < 10) {
          currentVideoSec = "0".concat(currentVideoSec);
        }

        $('.video-controls .plyr__time.plyr__time--current').html("".concat(currentVideoMin) + ':' + "".concat(currentVideoSec));
        $('.timeline-current-time').html("".concat(currentVideoMin) + ':' + "".concat(currentVideoSec));
        var timelinePercent = players[0].currentTime / players[0].duration;
        var videoTimelineContainer = document.querySelector('.timeline-container');
    ...
    var limit = 30; // players.forEach(function (instance, index) {
    // 	instance.volume = '0.1';
    // });
